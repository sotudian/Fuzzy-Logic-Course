...
for i=1:1:101
    alfa=(i-1)/100;
   y(i)=alfa;
s1L(i)=min(((a2L-a1L)*alfa+a1L),((b2L-b1L)*alfa+b1L));
s2L(i)=min((-(a4L-a3L)*alfa+a4L),(-(b4L-b3L)*alfa+b4L));

s1U(i)=min(((a2U-a1U)*alfa+a1U),((b2U-b1U)*alfa+b1U));
s2U(i)=min((-(a4U-a3U)*alfa+a4U),(-(b4U-b3U)*alfa+b4U));
...
plot(l,h1,'g','linewidth',1.5)
hold on
h2=trapmf(l,[2 6 8 11]);
plot(l,h2,'g','linewidth',1.5)
...
p=ones([size(s3U),1]);
plot(s3U,p,'y','linewidth',2)

axis([0,13,0,1.1])
title('MIN  -------Type II Trapezoidal fuzzy Number')
