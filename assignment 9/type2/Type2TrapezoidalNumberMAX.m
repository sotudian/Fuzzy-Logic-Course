...
for i=1:1:101
    alfa=(i-1)/100;
   y(i)=alfa;
s1L(i)=max(((a2L-a1L)*alfa+a1L),((b2L-b1L)*alfa+b1L));
s2L(i)=max((-(a4L-a3L)*alfa+a4L),(-(b4L-b3L)*alfa+b4L));

s1U(i)=max(((a2U-a1U)*alfa+a1U),((b2U-b1U)*alfa+b1U));
s2U(i)=max((-(a4U-a3U)*alfa+a4U),(-(b4U-b3U)*alfa+b4U));
...
p=ones([size(s3U),1]);
plot(s3U,p,'b','linewidth',2)

axis([0,13,0,1.1])
title('MAX    -------Type II Trapezoidal fuzzy Number')
