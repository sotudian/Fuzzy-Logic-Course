...
for i=1:1:101
    alfa=(i-1)/100;
   y(i)=alfa;
s1U(i)=min((mean1U-sigma1U*sqrt((-2)*log(alfa))),(mean2U-sigma2U*sqrt((-2)*log(alfa))));
s2U(i)=min((mean1U+sigma1U*sqrt((-2)*log(alfa))),(mean2U+sigma2U*sqrt((-2)*log(alfa))));

s1L(i)=min((mean1L-sigma1L*sqrt((-2)*log(alfa))),(mean2L-sigma2L*sqrt((-2)*log(alfa))));
s2L(i)=min((mean1L+sigma1L*sqrt((-2)*log(alfa))),(mean2L+sigma2L*sqrt((-2)*log(alfa))));
end
...
fill(X,Y,'b');
      ...
      subplot(313)
plot(s1L,y,'g','linewidth',2)
hold on
plot(s2L,y,'g','linewidth',2)
hold on
plot(s1U,y,'g','linewidth',2)
hold on
plot(s2U,y,'g','linewidth',2)
axis([3,13,0,1.1])
hold on
...
title('MIN   -------Type II Gaussian fuzzy Number')
