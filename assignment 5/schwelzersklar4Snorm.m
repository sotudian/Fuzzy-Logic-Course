...
%  schwelzer-sklar 4 S-Norm
clear
clc
z=[];
p=0;
disp('*** schwelzer sklar 4 S-Norm for Gaussian membership function ***')
...
disp('Enter the parameter of Yager t-Norm -->   p (p>0) ***')
p=input('Enter the amount of  p ');
...
for l=1:i
    for k=1:j
      z(l,k)=1-(((1-Y1(l,k))*(1-Y2(l,k)))/((((1-Y1(l,k))^(p))+((1-Y2(l,k))^(p))-((1-Y1(l,k))^(p))*((1-Y2(l,k))^(p)))^(1/p)));
      end
end
...
ylabel('b')
zlabel('S-Norm')
title('schwelzer-sklar-4 S-Norm for Gaussian MF ');
colormap parula
