...
%  schwelzer-sklar 2 S-Norm
clear
clc
z=[];
p=0;
disp('*** schwelzer sklar 2 S-Norm for Gaussian membership function ***')
...
for l=1:i
    for k=1:j
      z(l,k)=((((Y1(l,k))^p)  +  ((Y2(l,k))^p) - ((Y1(l,k))^p)*((Y2(l,k))^p)  )^(1/p));
...
ylabel('b')
zlabel('S-Norm')
title('schwelzer-sklar-2 S-Norm for Gaussian MF ');
colormap cool
