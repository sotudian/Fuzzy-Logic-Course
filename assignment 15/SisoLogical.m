...
for i=1:1:10
 ...
 disp('Please enter the amount of ALK Phosphate')
  ...
  % compelemnt of Antecedents
                  for d=1:10
                 ComAntecedent(d,:)=1-Antecedent(d,:);
                  end   
     for e=1:10
    for j=1:30001
       w(j)= min(A(j),ComAntecedent(e,j));
    end
    DOF(e)=max(w);
    end
    ...
    for e=1:10
    for j=1:10001
       f(e,j)= max(DOF(e),Consequent(e,j));
    end
     end
...
%% Plotting
        ...
        for i=1:10
        subplot(11,2,j)
        plot(L,ComAntecedent(i,:),'b','linewidth',1.5)
        ...
        for i=1:10
        subplot(11,2,j)
        plot(S,f(i,:),'g','linewidth',1.5)
       title(['Output',' -- ','Rule ',num2str(i)])
        j=j+2;
             hold on
        end
        subplot(11,2,21)
plot(L,A,'k','linewidth',1.5)
title('Initial Input')
subplot(11,2,22)
plot(S,AGGRIGATE,'r','linewidth',1.5)
title('AGGRIGATE')
